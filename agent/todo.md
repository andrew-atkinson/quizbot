...
- _New:_ **visually-oriented question types** — questions built around an image (identify/compare/annotate a photo, diagram, or artwork), not just text. **L.** Needs an image-bearing variant kind in `bank.py`, a QTI representation (Canvas supports images in question stems), and instructor-sourced images (same no-invented-URL rule as page embeds). Ties to the art/photo courses and the deck-image extraction item.
