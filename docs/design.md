...
| ----------------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------ |
| **bauhaus** _(default)_ | Form follows function                    | Geometric sans, heavy weight, thick cobalt bars on headings                                      |
...
- **Canvas property allowlist.** Every inline style the renderer emits uses a CSS property Canvas's own sanitizer keeps (transcribed from `canvas_sanitize.rb`). A theme literally cannot emit styling Canvas would strip.
- **WCAG AA contrast.** Ink, accent-as-text, tints, segment chips, code, and any surface all clear 4.5:1 — validated against each theme's own ground (terminal's dark petrol, not white). An unreadable accent override is rejected with a warning and falls back.
- **Accessibility of media.** Every image ships with alt text (an image with no alt or caption is dropped, not shipped inaccessible); every embed iframe carries a title.
